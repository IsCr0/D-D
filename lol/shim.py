...
def update(cycle,check,event_number,x):
 #idle
  ...
  #window.geometry('100x100+'+str(x)+'+850')
  window.geometry('100x100+'+str(x)+'+'+str(window.winfo_screenheight()-140))
...
walk_positive = [tk.PhotoImage(file=impath+'walking_left.gif',format = 'gif -index %i' %(i)) for i in range(8)]#walk to left gif
walk_negative = [tk.PhotoImage(file=impath+'walking_right.gif',format = 'gif -index %i' %(i)) for i in range(8)]#walk to right gif
